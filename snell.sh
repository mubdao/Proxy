...
# 仅检测当前内核是否已开启 TFO 服务端支持，作为提示信息展示给用户参考；
# 不在此处修改任何 sysctl，系统层面的 TFO 开启由用户自己的其他脚本负责。
check_kernel_tfo_status() {
    local val
    val=$(sysctl -n net.ipv4.tcp_fastopen 2>/dev/null || cat /proc/sys/net/ipv4/tcp_fastopen 2>/dev/null || echo "")
...
configure_tfo() {
    ...
    echo -e "\n${YELLOW}>>>> 配置 TCP Fast Open (TFO) <<<<${NC}"

    local kernel_status
    kernel_status=$(check_kernel_tfo_status)
    case "$kernel_status" in
        enabled)
            log_success "内核层面已开启 TFO 服务端支持 (net.ipv4.tcp_fastopen)。"
            ;;
        disabled:*)
            log_warn "内核层面尚未开启 TFO 服务端支持 (当前值: ${kernel_status#disabled:})。"
            log_warn "即使这里启用，也需要内核开启后才会真正生效，本脚本不会替你修改 sysctl。"
            ;;
        *)
            log_warn "无法读取内核 TFO 状态，请自行确认 net.ipv4.tcp_fastopen 是否已开启。"
            ;;
    esac

    echo -e "${CYAN}-----------------------------------------------------${NC}"
    local tfo_opt
    read -rp " 是否在 Snell 配置中启用 TFO？[y/N]: " tfo_opt
    if [[ "$tfo_opt" =~ ^[Yy]$ ]]; then
        SNELL_TFO="true"
    fi
}
...
install_node() {
    ...
    jq -n \
        ...
        --arg mode "$SNELL_MODE" \
        --arg tfo "$SNELL_TFO" \
        '{version:$version,release_name:$name,port:$port,psk:$psk,arch:$arch,mode:$mode,tfo:$tfo}' > "$INFO_PATH"
    chmod 600 "$INFO_PATH"
...
update_snell_version() {
    ...
    local current_version current_port current_psk current_mode current_arch current_tfo
    ...
    current_tfo=$(jq -r '.tfo // ""' "$INFO_PATH" 2>/dev/null || true)
    current_arch=$(jq -r '.arch // ""' "$INFO_PATH" 2>/dev/null || true)
    if [[ -z "$current_arch" ]]; then
        current_arch=$(get_arch) || { pause; return; }
    fi
    ...
    echo -e "${CYAN}-----------------------------------------------------${NC}"
    echo -e " ${GREEN}1.${NC} 确认更新（端口 / PSK / 模式 / TFO 保持不变）"
    ...
    # 配置文件（端口 / PSK / mode / tfo）保持不变，只更新记录的版本号。
    jq -n \
        --arg version "$RELEASE_TAG" \
        --arg name "$RELEASE_NAME" \
    ...
    log_success "Snell 已成功更新至 ${RELEASE_TAG}（端口 / PSK / 模式 / TFO 保持不变）。"
    pause
}
...
show_links() {
    ...
    # Surge 客户端的 tfo 参数与服务端配置里的 tfo 是两个独立开关，
    # 服务端已启用时，客户端配置也一并追加，保持两端一致。
    local tfo_suffix=""
    [[ "$tfo" == "true" ]] && tfo_suffix=", tfo=true"

    if [[ "$major" == "6" && -n "$mode" ]]; then
        echo -e "${YELLOW}Snell_${ip} = snell, ${ip}, ${port}, psk=${psk}, version=6, mode=${mode}${tfo_suffix}${NC}"
    elif [[ "$major" == "6" ]]; then
        echo -e "${YELLOW}Snell_${ip} = snell, ${ip}, ${port}, psk=${psk}, version=6${tfo_suffix}${NC}"
    else
        echo -e "${YELLOW}Snell_${ip} = snell, ${ip}, ${port}, psk=${psk}, version=${major}${tfo_suffix}${NC}"
    fi

    echo -e "\n${CYAN}版本: ${version}${NC}"
    [[ -n "$mode" ]] && echo -e "${CYAN}模式: ${mode}${NC}"
    [[ "$tfo" == "true" ]] && echo -e "${CYAN}TFO: 已启用（服务端 + 客户端配置均已启用）${NC}"
    echo -e "${CYAN}=====================================================${NC}"
    pause
}
