...
force_register_shortcut() {
    ...
    if [[ -s "$0" && "$0" != "bash" && "$0" != "/bin/bash" && "$0" != "/usr/bin/bash" ]]; then
        cp -f "$0" "$LOCAL_SCRIPT_PATH" 2>/dev/null || true
    fi

    chmod +x "$LOCAL_SCRIPT_PATH" 2>/dev/null || true

    # 2. 直接在系统的全局命令目录生成实体可执行文件 /usr/local/bin/any 与 /usr/bin/any
    ...
    for path in "${target_paths[@]}"; do
        ...
        # 写入仅依赖本地 /root/any.sh 的唤醒指令
        cat << 'EOF' > "$path"
#!/usr/bin/env bash
if [[ -s /root/any.sh ]]; then
    exec bash /root/any.sh "$@"
else
    echo -e "\033[0;31m[ERROR]\033[0m 未找到 /root/any.sh，请先运行本脚本一次。"
    exit 1
fi
EOF
...
install_node() {
    ...
    t_inbound=$(jq -n \
        ...
        '{
            ...
            users: [{ password: $w }],
            padding_scheme: ["stop=8","0=30-80","1=100-400","2=400-500,c,500-1000,c,500-1000,c,500-1000","3=9-9,500-1000","4=500-1000","5=500-1000","6=500-1000","7=500-1000"],
            tls: {
                enabled: true,
