...
config_ensure_exists() {
  ...
  # 【修复】config_min_template / config_normalize 里把日志路径写死为
  # /var/log/sing-box/access.log，但 sing-box 只会自动创建日志文件，
  # 不会自动创建父目录；目录不存在时会在启动阶段 FATAL 退出
  # （start service: start logger: open ...: no such file or directory）。
  # 这里每次进入管理菜单都会确保该目录存在，防止因为目录被误删/系统重装等
  # 原因导致 sing-box 起不来。
...
ss_link_for_inbound() {
  ...
  echo "节点：${tag}"
  echo "  [通用 ss://] ss://${b64}@${ip}:${port}#${tag}"
  echo "  [Surge]      Proxy-${tag} = ss, ${ip}, ${port}, encrypt-method=${method}, password=${password}"
}
...
# 所以没有"通用 URI"这一项；但 Surge（iOS 5.17.0+ / Mac 6.4.3+）已经原生支持
# AnyTLS 策略语法，这里按 Surge 官方文档的字段顺序给出可以直接粘贴的一行配置：
# https://manual.nssurge.com/policy/proxy.html
# 因为证书是脚本生成的自签证书，Surge 端必须加 skip-cert-verify=true 跳过验证，
# 否则会握手失败——这不是本脚本的私有行为，是自签证书场景下的通用要求。
anytls_show_inbound() {
  ...
  echo "  密码: ${password}"
  echo "  SNI : ${sni}"
  echo "  [Surge] Proxy-${tag} = anytls, ${ip}, ${port}, password=${password}, sni=${sni}, skip-cert-verify=true"
  echo "  客户端 outbound JSON 片段（其它支持 AnyTLS 的客户端可参考此结构填参数，自签证书需开启 insecure/allow_insecure）："
...
anytls_modify() {
  ...
  case "${act:-}" in
    ...
    2)
      ...
      ;;
    3)
      anytls_reselect_sni "$json" "$tag" "$cur_port"
      ;;
    0|"") return 0 ;;
    *) warn "无效选择"; ;;
  esac
...
# “重新选择 SNI”：直接调用跟 anytls_add() 装机时完全相同的 choose_tls_domain()
# 函数（不再单独维护一份"选域名"菜单，避免出现两份逻辑不一致的实现），
# 选出新域名后，用原版 ensure_self_signed_cert 针对新 SNI 重新签发自签证书
# （证书路径不变，仍是 /etc/sing-box/anytls-<port>.crt/.key，内容用新 CN=新SNI
# 重新生成），最后只更新 inbound 里的 tls.server_name。
anytls_reselect_sni() {
  local json="$1" tag="$2" port="$3"
  clear
  ...
  local new_sni
  new_sni="$(choose_tls_domain "AnyTLS")" || { warn "已取消。"; return 1; }
...
install_singbox_service() {
  # 同样在安装阶段确保日志目录存在，见 config_ensure_exists() 里的说明。
  mkdir -p /var/log/sing-box
  case "$INIT_SYSTEM" in
    systemd)
