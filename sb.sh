...
configure_node() {
    ...
    echo -e "${CYAN}=====================================================${NC}"
    local anytls_tag="" ss_tag="" snell_tag=""
    if [[ -f "$INFO_PATH" ]]; then
        jq -e .anytls "$INFO_PATH" >/dev/null 2>&1 && anytls_tag=" ${GREEN}(已部署)${NC}"
        jq -e .ss2022 "$INFO_PATH" >/dev/null 2>&1 && ss_tag=" ${GREEN}(已部署)${NC}"
        jq -e .snell  "$INFO_PATH" >/dev/null 2>&1 && snell_tag=" ${GREEN}(已部署)${NC}"
    fi

    echo -e " 1. 部署 ${BOLD}AnyTLS${NC}${anytls_tag}"
    echo -e " 2. 部署 ${BOLD}SS2022${NC}${ss_tag}"
    echo -e " 3. 部署 ${BOLD}Snell v6${NC}${snell_tag}"
    echo -e " 0. 返回主菜单"
    echo -e "${CYAN}-----------------------------------------------------${NC}"
