...
print_system_status() {
    ...
    if [[ -f "$CONFIG_PATH" && -f "$INFO_PATH" ]]; then
        ...
        if jq -e .anytls "$INFO_PATH" >/dev/null 2>&1 && jq -e .ss2022 "$INFO_PATH" >/dev/null 2>&1; then
            installed_proto="双协议 (AnyTLS + SS2022)"
        elif jq -e .anytls "$INFO_PATH" >/dev/null 2>&1; then
            installed_proto="AnyTLS"
        elif jq -e .ss2022 "$INFO_PATH" >/dev/null 2>&1; then
            installed_proto="SS2022"
        else
...
configure_node() {
    ...
    echo -e "${CYAN}=====================================================${NC}"
    echo -e " 1. 单独部署 ${BOLD}AnyTLS${NC}"
    echo -e " 2. 单独部署 ${BOLD}SS2022${NC}"
    echo -e " 3. 双协议同时部署 (${BOLD}AnyTLS + SS2022${NC})"
    echo -e " 0. 返回主菜单"
    echo -e "${CYAN}-----------------------------------------------------${NC}"
    ...
    if [[ "$mode" == "2" || "$mode" == "3" ]]; then
        ...
        info_json=$(echo "$info_json" | jq --arg p "$ss_port" --arg m "$ss_method" --arg w "$ss_pwd" \
            '. + {ss2022: {port: $p, method: $m, pwd: $w}}')
...
show_links() {
    ...
    if jq -e .anytls "$INFO_PATH" >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW}AnyTLS_${ip} = anytls, ${ip}, ${p}, password=${w}, tls=true, sni=${s}, skip-cert-verify=true${NC}\n"
    fi
    ...
    if jq -e .ss2022 "$INFO_PATH" >/dev/null 2>&1; then
        ...
        ss_uri="ss://${ss_userinfo}@${ip}:${ss_p}#SS2022_${ip}"

        echo -e "${GREEN}[ SS2022 节点 ]: ${NC}"
        echo -e "${YELLOW}${ss_uri}${NC}\n"

        echo -e "${GREEN}[ SS2022 节点 (Surge) ]: ${NC}"
        echo -e "${YELLOW}SS2022_${ip} = ss, ${ip}, ${ss_p}, encrypt-method=${ss_m}, password=${ss_w}${NC}\n"
    fi
