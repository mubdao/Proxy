...
C='\033[1;36m'; NC='\033[0m'; W='\033[1;37m'; GRAY='\033[2m'
...
ss_pick_entry_or_return() {
  ...
  if [ ${#lines[@]} -eq 1 ]; then
    local _ek _proto _port
    IFS=$'\x01' read -r _ek _proto _port <<< "${lines[0]}"
    printf -v "$outvar" '%s' "$_ek"
    return 0
  fi
  local i=1 _ek _proto _port
  echo "当前 Shadowsocks 节点："
  for line in "${lines[@]}"; do
    IFS=$'\x01' read -r _ek _proto _port <<< "$line"
    echo "  [$i] ${_ek}  端口:${_port}"
    i=$((i+1))
  done
  local choice
  ...
  IFS=$'\x01' read -r _ek _proto _port <<< "${lines[$((choice-1))]}"
  printf -v "$outvar" '%s' "$_ek"
}
...
main_menu() {
  while true; do
    clear
    main_menu_header
    local _json _ss_present _at_present
    _json="$(config_load 2>/dev/null || echo '{}')"
    ss_exists "$_json" && _ss_present=1 || _ss_present=0
    anytls_exists "$_json" && _at_present=1 || _at_present=0

    echo "----------------------------------------"
    if [ "$_ss_present" = "1" ]; then
      echo -e " ${GRAY} 1. 添加 Shadowsocks（已安装）${NC}"
    else
    ...
    echo
    if [ "$_at_present" = "1" ]; then
      echo -e " ${GRAY} 4. 添加 AnyTLS（已安装）${NC}"
    else
      echo " 4. 添加 AnyTLS"
    fi
    echo " 5. 修改 AnyTLS"
    ...
    case "${act:-}" in
      1)
        if [ "$_ss_present" = "1" ]; then
          warn "Shadowsocks 已安装，如需修改请使用「2. 修改 Shadowsocks」。"
          pause
        else
          ss_add
        fi
      ...
      4)
        if [ "$_at_present" = "1" ]; then
          warn "AnyTLS 已安装，如需修改请使用「5. 修改 AnyTLS」。"
          pause
        else
          anytls_add
        fi
